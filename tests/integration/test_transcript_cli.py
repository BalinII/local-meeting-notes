...
def test_cli_transcript_commands(local_tmp_dir, monkeypatch) -> None:
    ...
    try:
        ...
        with redirect_stdout(out):
            ...
            assert main(["transcript", "status", "--capture-id", capture_id]) == 0
            assert main(["transcript", "list", "--capture-id", capture_id]) == 0

        rendered = out.getvalue()
        assert "Completed chunks: 1" in rendered
